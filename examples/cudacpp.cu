...
namespace Examples::CUDACPP
{
    ...
    void transferDataAsyncPinnedDefaultStream()
    {
        ...
        // Transfer data from the host to the GPU.
        auto status = cudaMemcpyAsync(thrust::raw_pointer_cast(gpu_array.data()),
                                      thrust::raw_pointer_cast(host_array_input.data()),
                                      num_elements * sizeof(float), cudaMemcpyDefault);
        if (status != cudaSuccess)
        {
            throw std::runtime_error{
                "cudaMemcpyAsync failed " + std::to_string(status) + ": " +
                cudaGetErrorString(status)
            };
        }

        // Transfer data from the GPU to the host.
        status = cudaMemcpyAsync(thrust::raw_pointer_cast(host_array_output.data()),
                                 thrust::raw_pointer_cast(gpu_array.data()),
                                 num_elements * sizeof(float), cudaMemcpyDefault);
        if (status != cudaSuccess)
        {
            throw std::runtime_error{
    ...
    void transferDataAsyncPinnedDedicatedStream()
    {
        ...
        // Transfer data from the host to the GPU.
        auto status =
            cudaMemcpyAsync(thrust::raw_pointer_cast(gpu_array.data()),
                            thrust::raw_pointer_cast(host_array_input.data()),
        ...
        if (std::equal(host_array_input.begin(), host_array_input.end(),
                       host_array_output.begin()))
        {
            std::cout << "Data transfer is finished successfully." << std::endl;
        }
        else
