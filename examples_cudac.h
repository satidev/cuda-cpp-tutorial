...
inline void dataTransferPageable()
{
    ...
    if (status != cudaSuccess)
    {
        throw std::runtime_error{"cudaMemcpyHostToDevice failed-" + std::to_string(status) + ": " +
    ...
    if (status != cudaSuccess)
    {
        throw std::runtime_error{"cudaMemcpyDeviceToHost failed " + std::to_string(status) + ": " +
                                 cudaGetErrorString(status)};
    }

    if (std::equal(std::begin(host_array_input), std::end(host_array_input), std::begin(host_array_output)))
    {
...
inline void dataTransferPinnedStream()
{
    ...
    if (status != cudaSuccess)
    {
        throw std::runtime_error{"cudaHostAlloc failed " + std::to_string(status) + ": " + cudaGetErrorString(status)};
    }
    ...
    if (status != cudaSuccess)
    {
        throw std::runtime_error{"cudaStreamCreate failed " + std::to_string(status) + ": " +
                                 cudaGetErrorString(status)};
    }
